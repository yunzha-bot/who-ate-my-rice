...
**长期开发路线与下一阶段开发前置条件**（Git 基线与工作区保护、开发阶段授权、DEV-A / DEV-B 正式开发前的前置问题、S7C-1B 参数逐项批准、地图数据与随机化的唯一真相、测试与人工验收口径、唯一开发入口、现有功能保护清单）已整节收录在 `AGENTS.md` 的同名章节，**本文件不重复复制整套规则**，只在此引用；两者冲突时以 `AGENTS.md` 为准。
