...
日志语言与阶段归档格式统一遵守下方「前置条件 11：文档维护」；本节不另行维护重复规则。
...
- **开发日志与阶段归档规范**：Codex 与 DeepSeek Harness 均遵守本文件的日志语言及归档结构。每次阶段归档按「阶段名称与日期 → 完成内容 → 自动化测试与人工验收（表格）→ Git 检查点 → 已知限制与下一项任务」组织；正文中文优先，重要术语首次出现时附英文原名，代码标识符、配置键、路径和 SHA 保持准确。详细实现过程归对应设计文档，`HANDOFF` 只维护精简的当前状态，不复制长篇历史。归档时常规检查并同步相关文档，只依据源码、测试、验收与 Git 证据更正已确认的过期状态；不得猜测或将未验收功能写成完成。日志优化属于阶段归档的常规工作，不需额外创建任务或为常规整理反复请求批准。若需改变长期权限/安全规则，或处理有争议、证据不足的历史事实，应先报告用户。该条不改变前置条件 1、2、10 中关于 commit、push、tag 的授权要求。
